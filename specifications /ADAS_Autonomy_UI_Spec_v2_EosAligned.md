...
## 10) Eos ADAS Demo Alignment (Qt Quick 3D “Eos ADAS” patterns)

This project suite **must incorporate** the following implementation patterns inspired by Qt’s public **Eos ADAS** demo project (Qt 6 / Qt Quick 3D). These items are treated as **design constraints and feature expectations** for the ADAS/Autonomy UI experience in both Driver UI and Infotainment (while preserving the safety partitioning rules).

### 10.1 Reference assets and project structure expectations
Your implementation should support an “Eos-style” structure and tooling flow:

- **Qt Quick (QML) entrypoint** with `main.qml` and an app project file (e.g., `*.qmlproject`) for quick iteration.
- **Resource packaging** via `*.qrc` and a top-level screenshot/preview asset (e.g., `*.png`) for documentation and release notes.
- A CMake build that can produce the same app **without** Design Studio, enabling CI builds and multi-platform packaging.

> Rationale: the Eos ADAS demo is distributed as a runnable UI project and is also consumable via Qt Design Studio workflows.

### 10.2 Environmental “look presets” for rapid scenario evaluation (mandatory)
Provide at least four preset lighting/environment states selectable at runtime (desktop: keyboard; in-vehicle: engineering menu behind auth):
- Dawn
- Day
- Dusk
- Night
...
**Rules**
- Switching presets must be **non-blocking** and must not cause missed takeover/MRM alerts.
- Preset selection must be logged to `HmiEventLog` (engineering only).

### 10.3 Weather particle effects (informational + ODD cues)
Implement particle-based weather effects (rain/snow/mist) as a **situational awareness** channel and as an **ODD / degraded-mode cue** (e.g., “Limited due to heavy rain”).
- Effects must be scalable by `AdasVisualQualityManager` (reduce particle budget first).
- Driver UI must not rely on particles for safety-critical state recognition; particles are an *augmentation* only.

### 10.4 Realtime reflections (aesthetic, quality-scaled)
Provide reflection realism using reflection probes where supported.
- Reflections are **non-safety** aesthetics; they are always disabled first in degraded performance modes.
- Reflection update rate must be adjustable by `AdasVisualQualityManager`.

### 10.5 Skeletal animation support (for living scenes / object realism)
Where 3D scene objects require animation (e.g., wipers, suspension, pedestrian rigs), support skeletal animation in the 3D pipeline.
- Skeletal animation is optional for Driver UI, but recommended for the Infotainment hero view.
- Animation must degrade gracefully (reduce bone updates / switch to baked animation) before any safety cues are affected.

### 10.6 Scenario/event replay tooling (Eos-style “Event List”)
Create a scenario/event replay capability modeled after an “event list simulator” concept:
- Provide an **Event List Model** (sequence of timed events) and an **Event List Simulator** (play/pause/seek).
- The simulator must drive the ADAS model inputs (lanes, objects, ODD changes, takeover requests) in the same way as live IPC topics.

**UI integration**
- Infotainment: “Perception Explorer (Engineering)” includes Event List controls (auth + motion locked).
- Driver UI: never exposes simulator controls; only shows resulting state for verification builds.

### 10.7 Qt Design Studio friendliness
Maintain a “Design Studio-compatible” structure for the ADAS 3D scene:
- Keep heavy 3D assets under a dedicated `asset_imports/Quick3DAssets/...` hierarchy.
- Provide designer metadata (`designer/plugin.metainfo`) for any custom QML components used for scenario tooling (e.g., EventListSimulator) so they can be discovered and placed from the Design Studio navigator.

### 10.8 Complexity / performance expectations (design-time and runtime)
Use the Eos ADAS Design Studio project size as a **realistic upper-bound** for complexity of a single ADAS view:
- Target: the main ADAS view should remain roughly in the same order of magnitude of object count as a large QDS ADAS scene (hundreds of objects, not thousands).
- Prefer instancing, shared materials, and LOD to keep GPU/CPU budgets stable.
